...
\href{https://github.com/pinking-jeong/Nonproliferation}{github.com/pinking-jeong/Nonproliferation}. Datasets:
...
\begin{table}[h]
  \centering
  \small
  \caption{Per-case retrofit results across all eight historical cases.
  ``Pass'' applies the $\text{posterior} \geq 0.4$ rule introduced in
  Section~\ref{sec:fusion}. The four upper rows are the original Phase~1
  cases; the four lower rows are voluntarily-disclosed historical
  programmes added in Phase~2 to improve statistical power and
  calibration. \emph{ECE} is a one-bin reliability estimate.}
  \label{tab:per_case}
  ...
  \midrule
  South Africa pre-1991           & \cmark & \cmark & 0.67 & 0.894 & \cmark \\
  Argentina pre-1991              & \cmark & \cmark & 0.67 & 0.544 & \cmark \\
  Brazil pre-1991                 & \cmark & \cmark & 0.67 & 0.544 & \cmark \\
  Taiwan pre-1988                 & \cmark & \cmark & 0.67 & 0.836 & \cmark \\
  \midrule
  \textbf{Macro mean (all 8)}     & \textbf{1.00} & \textbf{1.00} & \textbf{0.49} & \textbf{0.498} & 5/8 \\
  Phase~1 sub-aggregate (4)       & 1.00 & 1.00 & 0.31 & 0.291 & 1/4 \\
  Phase~2 sub-aggregate (4)       & 1.00 & 1.00 & 0.67 & 0.704 & 4/4 \\
  \bottomrule
  \end{tabular}
  \\[2pt]\footnotesize
  Aggregate ECE drops from $0.71$ (Phase~1 4-case set) to $0.50$
  (8-case set) because the Phase~2 cases have richer public-domain
  signature density --- their post-disclosure documentation in
  ABACC, IAEA, and US-State-Department archives is more substantial
  than the still-active or recently-disclosed cases of Phase~1.
\end{table}

\paragraph{Phase~2 calibration improvement.}
Adding four voluntarily-disclosed cases (South Africa pre-1991,
Argentina pre-1991, Brazil pre-1991, Taiwan pre-1988) lifts the
mean posterior from $0.291$ to $0.498$ and reduces ECE from $0.71$
to $0.50$. All four Phase~2 cases pass the $\text{posterior}
\geq 0.4$ threshold. The asymmetry between Phase~1 and Phase~2 sub-
aggregates is informative: cases for which detailed post-disclosure
documentation is available in the open literature (ABACC, IAEA
post-1991 inspections, US State Department records) yield denser
indicator clusters than cases that are still partially classified or
politically sensitive (Iran-Natanz, Syria-Al-Kibar). This is a
property of the public-domain corpus, not of the system itself.

\paragraph{Imagery modality lifts confidence dramatically (Phase~2 COG ablation).}
We additionally re-ran Module~A on a 5\,km $\times$ 5\,km AOI cropped
from the Sentinel-2 visual COG (TCI.tif, 10\,m GSD, scene
\texttt{S2A\_39SWT\_20241229\_0\_L2A}) over the Natanz facility,
contrasted against the same Module~A on the 70\,KB STAC \emph{thumbnail}
preview. Confidence rises from $0.10$ (\texttt{facility\_type:
uncertain}) on the thumbnail to $0.85$ (\texttt{facility\_type:
enrichment}) on the COG crop, with five named visual evidence items
(``cascade halls,'' ``security perimeter,'' ``berms''). This validates
the Phase~1 hypothesis that the imagery modality, once given GSD
sufficient to resolve facility-scale features, materially closes the
calibration gap.
...
\begin{table}[h]
  ...
  \midrule
  Heuristic only (4 cases)         & 1.00 & 1.00 & 0.291 & --- \\
  + Fusion (Bayesian, 4 cases)     & 1.00 & 1.00 & 0.291 & --- \\
  + Module E (APA risk)            & 1.00 & 1.00 & 0.291 & --- \\
  + GNN-light, LR (LOO)            & 1.00 & 1.00 & 0.291 & 1.00 \\
  + GNN-light, GBM (LOO)           & 1.00 & 1.00 & 0.291 & 1.00 \\
  + VLM (live, 4 cases)            & 1.00 & 1.00 & 0.182 & --- \\
  \textbf{+ Phase~2 retrofit (8 cases)} & \textbf{1.00} & \textbf{1.00} & \textbf{0.498} & --- \\
  \textbf{+ Imagery COG (Natanz, 1 scene)} & --- & --- & \textbf{conf 0.85} & --- \\
  \bottomrule
  \end{tabular}
  \\[2pt]\footnotesize
  GNN-light values are leave-one-out across the 4 Phase~1 cases against
  synthetic indicator-stripped negatives (\S\ref{sec:gnnlite}). The
  Phase~2 retrofit row aggregates 8 cases (Phase~1 set + 4 voluntarily-
  disclosed historical programmes: South Africa, Argentina, Brazil,
  Taiwan). The Imagery~COG row reports a single Natanz scene processed
  by cropping the Sentinel-2 visual COG (TCI.tif) to a 5\,km $\times$
  5\,km AOI and running Module~A; the per-tile cost was about \$0.005.
  We also catalogue 33 IAEA Broader-Conclusion (BC) states as
  realistic-negative reference candidates for forthcoming GNN-light
  retraining (Section~\ref{sec:disc}).
\end{table}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item Code: \url{https://github.com/pinking-jeong/Nonproliferation} (Apache-2.0).
